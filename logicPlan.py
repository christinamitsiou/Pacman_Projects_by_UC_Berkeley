...
def sentence1() -> Expr:
    ...
    Not_B_or_C = Not_B | C      # (not B) or C
   
    Not_A_if_only_not_B_or_C = Not_A % Not_B_or_C       # (not A) if and only if ((not B) or C)
    
    Not_A_or_Not_B = Not_A | Not_B      # (not A) or (not B)
    
    Not_A_or_Not_B_or_C = disjoin([Not_A_or_Not_B,C])    # (not A) or (not B) or C
    
    result = conjoin([A_or_B,Not_A_if_only_not_B_or_C, Not_A_or_Not_B_or_C])
...
def sentence2() -> Expr:
    ...
    not_B_and_not_D = Not_B & Not_D      # (not B) and (not D)
    
    B_and_not_C = B & Not_C              # B and (not C)

    Not_B_and_not_C = ~ B_and_not_C      # not (B and (not C))
        
    C_if_and_only_B_or_D = C % B_or_D    # C if and only if (B or D)
    
    A_implies_not_B_and_not_C = A >> not_B_and_not_D        # A implies ((not B) and (not D))

    Not_B_and_not_C_implies_A = Not_B_and_not_C >> A        # (not (B and (not C))) implies A

    Not_D_implies_C = Not_D >> C
    
    result = conjoin([C_if_and_only_B_or_D,A_implies_not_B_and_not_C,Not_B_and_not_C_implies_A,Not_D_implies_C])
...
def sentence3() -> Expr:
    ...
    Alive_at_1 = PropSymbolExpr("PacmanAlive_1")
    Alive_at_0 = PropSymbolExpr("PacmanAlive_0")
    Born_at_0 = PropSymbolExpr("PacmanBorn_0")
    Killed_at_0 = PropSymbolExpr("PacmanKilled_0")
    
    # Pacman was alive at time 0 and was not killed at time 0
    Str1 = Alive_at_0 & ~ Killed_at_0
    
    # Pacman was not alive at time 0 and it was born at time 0
    Str2 = ~Alive_at_0 & Born_at_0
    
    # Pacman was alive at time 0 and it was not killed at time 0) or (it was not alive at time 0 and it was born at time 0
    Str3 = Str1 | Str2
    ...
    Str4 = Alive_at_1 % Str3
    
    # # Pacman cannot both be alive at time 0 and be born at time 0.
    Str5 = ~(Alive_at_0 & Born_at_0)
   
    result = conjoin([Str4,Str5,Born_at_0])
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    # Get combinations of 2 literals 
    combinations = itertools.combinations(literals,2)
    
    # For each combination 
    for literal_1, literal_2 in combinations:

            # If they're different, we disjoin ~literal_1 and ~literal_2 and add that to the list
            
            if literal_1 != literal_2:
                
                # If both literals are True the expression (~literal_1 or ~literal_2) is False
                # If only one literal is true/if no literal is true (we have at most one true) the expression is True 
                at_most_one.append(disjoin(~literal_1 ,~literal_2))
    
    # We conjoin the list
    return conjoin(at_most_one)
    # util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    # At least one expression is True & no more than one expression is True
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    expr = PropSymbolExpr(pacman_str, x, y, time = now) % causes
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    # All of Pacman's possible positions
    possible_pos = []
    
    # All of Pacman's possible directions
    possible_directions = []

    # We check each pair of coordinates to make sure Pacman is not at the position of a wall
    for coordinate in all_coords:
        x = coordinate[0]
        y = coordinate[1] 
        
        # If there's a wall at x,y  Pacman can not be at x,y at any time
        pacphysics_sentences.append(PropSymbolExpr(wall_str, x, y) >> ~PropSymbolExpr(pacman_str, x, y, time=t))
    
    # We check the inside of the walls, were Pacman can be
    for coordinate in non_outer_wall_coords:
        x = coordinate[0]
        y = coordinate[1]
        
        # Each pair of coordinates inside the walls is a possible Pacman position
        possible_pos.append(PropSymbolExpr(pacman_str, x, y, time=t))
        
    # Pacman can only be at one of the possible positions at a certain time
    pacphysics_sentences.append(exactlyOne(possible_pos))

    # We check all four directons where Pacman can go 
    for dir in DIRECTIONS:
        possible_directions.append(PropSymbolExpr(dir, time=t))
        
    # Pacman can only move to one direction at a certain time
    pacphysics_sentences.append(exactlyOne(possible_directions))
    
    # If sensorModel is provided, we use it
    if sensorModel != None:
        pacphysics_sentences.append(sensorModel(t, non_outer_wall_coords))
    
    # If sensorModel is provided, and time is not 0, we use it
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    # Pacmans's physics axioms for timestamp = 0
    physics_0 = pacphysicsAxioms(0, all_coords, non_outer_wall_coords)
    KB.append(physics_0)

    # Pacmans's physics axioms for timestamp = 1
    physics_1 = pacphysicsAxioms(1, all_coords, non_outer_wall_coords)
    KB.append(physics_1)

    KB.append(allLegalSuccessorAxioms(1,walls_grid, non_outer_wall_coords))
    
    # Pacman's position at timestamp = 0
    pacman_pos_0 = PropSymbolExpr(pacman_str, x0, y0, time=0)
    KB.append(pacman_pos_0)
    
    # Pacman takes action0 at tinestamp = 0 
    pacman_action_0 = PropSymbolExpr(action0, time=0)
    KB.append(pacman_action_0)
    
    # Pacman takes action1 at tinestamp = 1 
    pacman_action_1 = PropSymbolExpr(action1, time=1)
    KB.append(pacman_action_1)
    
    pos_1 = PropSymbolExpr(pacman_str, x1, y1, time=1)

    # A model where pacman is at the position with coordinates x1,y1 at time = 1, given everything in the KB list
    model_1 = findModel(conjoin(KB) & pos_1)
    # A model where pacman is not at the position with coordinates x1,y1 at time = 1, given everything in the KB list
    model_2 = findModel(conjoin(KB) & ~pos_1)
...
def positionLogicPlan(problem) -> List:
    ...
    # Add Pacman's original position at timestamp = 0 to the list 
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    
    for timestamp in range(50):
        
        # All of Pacman's possible positions at timestamp
        possible_locations = []
        
        # All of Pacman's possible directions at timestamp
        possible_directions = []
        ...
        for coordinate in non_wall_coords:
            ...
            possible_locations.append(PropSymbolExpr(pacman_str, x, y, time=timestamp))
            
            # If there's a wall at x,y, Pacman can not be at x,y 
            KB.append(PropSymbolExpr(wall_str, x, y) >> ~PropSymbolExpr(pacman_str, x, y, time=timestamp))
         
        # Pacman can only be at one of the possible positions at a certain time
        KB.append(exactlyOne(possible_locations))
        
        for action in actions:
            possible_directions.append(PropSymbolExpr(action, time=timestamp))
            
        # Pacman can only do one of the possible actions at a certain time
        KB.append(exactlyOne(possible_directions))
        ...
        if timestamp>0:
            ...
            for coordinate in non_wall_coords:
                ...
                # We call pacmanSuccessorAxiomSingle() for all possible positions at all timestamps except timestamp = 0, which is the initial position
                KB.append(pacmanSuccessorAxiomSingle(x, y, timestamp, walls_grid))
                
        # Pacman is at the goal at timestep t (goal has coordinates xg, yg).
        end_goal = PropSymbolExpr(pacman_str, xg, yg, time=timestamp)
        
        conjoined_KB = conjoin(KB)
        
        # A model where all of KB is true as well as the end goal
        model = findModel(conjoined_KB & end_goal)
        
        # If there is a model where the expression (conjoined_KB & end_goal) is true, we return a sequence of actions from start to goal 
        if model:
...
#     # Get all pairs of coordinates inside the walls
#     non_wall_coords = []
#     for coordinate in all_coords:
#             if coordinate not in walls_list:
#                 non_wall_coords.append(coordinate)
